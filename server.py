...
def remove_question(index,score):
    questions.pop(index)
    answers.pop(index)

    if len(answers) == 0:
        conn.send('Game Ended! Your final score is {}'.format(score).encode('utf-8'))
        return 'empty'
...
def clientthread(conn, nickname):
    ...
    while True:
        try:
            ...
            print('{} finished the quiz!'.format(nickname))
            break
...
while True:
    ...
    msg = '{} joined the quiz!'.format(nickname)
    print(msg)
    broadcast(msg,conn)

    clientThread = Thread(target=clientthread,args=(conn,nickname))
    clientThread.start()